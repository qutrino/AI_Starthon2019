...
from .py_utils.losses import CornerNet_Focal_Loss, CornerNet_Offset_Loss
from .py_utils.modules import hg_module, hg, hg_net, hg_net_offset
...
class model(hg_net_offset):
    ...
    def __init__(self):
        ...
        tl_heats = nn.ModuleList([self._pred_mod(1) for _ in range(stacks)]) # 80-> 1
        br_heats = nn.ModuleList([self._pred_mod(1) for _ in range(stacks)]) # 80-> 1
        ...
        super(model, self).__init__(
            hgs, tl_modules, br_modules, tl_heats, br_heats, tl_offs, br_offs )

        self.loss = CornerNet_Offset_Loss(pull_weight=1e-1, push_weight=1e-1)
